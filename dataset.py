...
def make_dataset(path, method="train"):
    ...
    real_soldier_dataset = ImageFolder(path[2], transform=data_transform)

    copy_forest_dataset = non_soldier_dataset.samples
    copy_r_soldier_dataset = real_soldier_dataset.samples
    ...
    if method == "train":  # 학습 시 실제 데이터 셋 사용
        ...
        copy_r_soldier_dataset = random.sample(real_soldier_dataset.samples, 700)
    ...
    if not set(soldier_has_subdirectory):
        ...
        for img_path, label in copy_r_soldier_dataset:
            img_filename = os.path.basename(img_path)
            shutil.copy2(img_path, os.path.join(path[4], img_filename))
